	...
	public void onTalkNpc(final Player player, final Npc n) {
		if (n.getID() == NpcId.WAYNE.id()) {
			...
			if (option == 0) {
				...
				if (player.getCarriedItems().getInventory().countId(ItemId.DRAGON_CHAIN_MAIL_BODY.id()) >= 500) {
					hasChains = true;
				...
				if (hasChains && hasScales) {
					...
					if (option2 == 0) {
						...
						if (player.getCarriedItems().remove(new Item(ItemId.CHIPPED_DRAGON_SCALE.id(), 150)) > -1) {
							if (player.getCarriedItems().remove(new Item(ItemId.DRAGON_CHAIN_MAIL_BODY.id() ,500)) > -1) {
