...
public class Smelting implements UseLocTrigger {
	...
	public void onUseLoc(Player player, GameObject obj, Item item) {
		if (obj.getID() == FURNACE && !DataConversions.inArray(new int[]{ItemId.GOLD_BAR.id(), ItemId.SILVER_BAR.id(), ItemId.SAND.id(), ItemId.GOLD_BAR_FAMILYCREST.id()}, item.getCatalogId())) {
			...
			int amount = 0;
			if (item.getCatalogId() == ItemId.DRAGON_LONG_SWORD.id())
				amount = 1;
			else if (item.getCatalogId() == ItemId.DRAGON_BATTLE_AXE.id())
				amount = 2;
			else {
	...
	private void handleRegularSmelting(final Item item, Player player, final GameObject obj) {
		if (!inArray(item.getCatalogId(),
				...
				Smelt.DRAGON_ORE.getID(), Smelt.KHORIUM_ORE.getID(), Smelt.PLATINUM_ORE.getID(), Smelt.TITANIUM_ORE.getID(),
	...
	private String smeltString(World world, Smelt smelt, Item item) {
		...
		} else if (smelt.getSmeltBarId() == ItemId.MITHRIL_BAR.id() || smelt.getSmeltBarId() == ItemId.ADAMANTITE_BAR.id()|| smelt.getSmeltBarId() == ItemId.DRAGON_BAR.id()|| smelt.getSmeltBarId() == ItemId.KHORIUM_BAR.id()|| smelt.getSmeltBarId() == ItemId.PLATINUM_BAR.id()|| smelt.getSmeltBarId() == ItemId.TITANIUM_BAR.id()|| smelt.getSmeltBarId() == ItemId.RUNITE_BAR.id()) {
	...
	enum Smelt {
		...
		DRAGON_ORE(ItemId.DRAGON_ORE.id(), 350, 100, 1, 408, 155, 10),
		KHORIUM_ORE(ItemId.KHORIUM_ORE.id(), 500, 115, 1, 1567, 155, 12),
		PLATINUM_ORE(ItemId.PLATINUM_ORE.id(), 750, 130, 1, 1569, 155, 14),
		TITANIUM_ORE(ItemId.TITANIUM_ORE.id(), 1000, 140, 1, 1571, 155, 16);
