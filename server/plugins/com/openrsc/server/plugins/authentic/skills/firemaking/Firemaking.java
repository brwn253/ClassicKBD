...
public class Firemaking implements UseObjTrigger, UseInvTrigger {
	...
	private static int[] LOGS = {ItemId.LOGS.id(), ItemId.OAK_LOGS.id(), ItemId.WILLOW_LOGS.id(),
			ItemId.MAPLE_LOGS.id(), ItemId.YEW_LOGS.id(), ItemId.MAGIC_LOGS.id(), ItemId.MYSTIC_LOGS.id(),
			ItemId.CORK_LOGS.id(), ItemId.BOOJUM_LOGS.id(), ItemId.GUM_LOGS.id(), ItemId.TWISTED_LOGS.id()};
