...
public class Drinkables implements OpInvTrigger {
	...
	public void onOpInv(Player player, Integer invIndex, Item item, String command) {
		...
		if (id == ItemId.GUJUO_POTION.id())
			...
			usePrayerPotion(player, item, ItemId.TWO_RESTORE_PRAYER_POTION.id(), 2, sendUpdate, 0.25);

		else if (id == ItemId.TWO_RESTORE_PRAYER_POTION.id())
			usePrayerPotion(player, item, ItemId.ONE_RESTORE_PRAYER_POTION.id(), 1, sendUpdate, 0.25);

		else if (id == ItemId.ONE_RESTORE_PRAYER_POTION.id())
			usePrayerPotion(player, item, ItemId.EMPTY_VIAL.id(), 0, sendUpdate, 0.25);
		...
		else if (id == ItemId.FULL_ULTIMATE_ATTACK_POTION.id())
			useNormalPotion(player, item, Skill.ATTACK.id(), 30, 6, ItemId.TWO_ULTIMATE_ATTACK_POTION.id(), 2, sendUpdate);

		else if (id == ItemId.TWO_ULTIMATE_ATTACK_POTION.id())
			useNormalPotion(player, item, Skill.ATTACK.id(), 30, 6, ItemId.ONE_ULTIMATE_ATTACK_POTION.id(), 1, sendUpdate);

		else if (id == ItemId.ONE_ULTIMATE_ATTACK_POTION.id())
			useNormalPotion(player, item, Skill.ATTACK.id(), 30, 6, ItemId.EMPTY_VIAL.id(), 0, sendUpdate);

		else if (id == ItemId.FULL_ULTIMATE_STRENGTH_POTION.id())
			useNormalPotion(player, item, Skill.STRENGTH.id(), 30, 6, ItemId.TWO_ULTIMATE_STRENGTH_POTION.id(), 2, sendUpdate);

		else if (id == ItemId.TWO_ULTIMATE_STRENGTH_POTION.id())
			useNormalPotion(player, item, Skill.STRENGTH.id(), 30, 6, ItemId.ONE_ULTIMATE_STRENGTH_POTION.id(), 1, sendUpdate);

		else if (id == ItemId.ONE_ULTIMATE_STRENGTH_POTION.id())
			useNormalPotion(player, item, Skill.STRENGTH.id(), 30, 6, ItemId.EMPTY_VIAL.id(), 0, sendUpdate);

		else if (id == ItemId.FULL_ULTIMATE_DEFENSE_POTION.id())
			useNormalPotion(player, item, Skill.DEFENSE.id(), 30, 6, ItemId.TWO_ULTIMATE_DEFENSE_POTION.id(), 2, sendUpdate);

		else if (id == ItemId.TWO_ULTIMATE_DEFENSE_POTION.id())
			useNormalPotion(player, item, Skill.DEFENSE.id(), 30, 6, ItemId.ONE_ULTIMATE_DEFENSE_POTION.id(), 1, sendUpdate);

		else if (id == ItemId.ONE_ULTIMATE_DEFENSE_POTION.id())
			useNormalPotion(player, item, Skill.DEFENSE.id(), 30, 6, ItemId.EMPTY_VIAL.id(), 0, sendUpdate);

		else if (id == ItemId.FULL_SUPER_PRAYER_POTION.id())
			usePrayerPotion(player, item, ItemId.TWO_SUPER_PRAYER_POTION.id(), 2, sendUpdate, 0.33);

		else if (id == ItemId.TWO_SUPER_PRAYER_POTION.id())
			usePrayerPotion(player, item, ItemId.ONE_SUPER_PRAYER_POTION.id(), 1, sendUpdate, 0.33);

		else if (id == ItemId.ONE_SUPER_PRAYER_POTION.id())
			usePrayerPotion(player, item, ItemId.EMPTY_VIAL.id(), 0, sendUpdate, 0.33);

		// FULL_WOODWORKING_POTION(1674) Not sure what these were supposed to do
		// TWO_WOODWORKING_POTION(1675)
		// ONE_WOODWORKING_POTION(1676)
		// FULL_ULTIMATE_ZAMORAK_POTION(1689) need to look at how we want this to work
		// TWO_ULTIMATE_ZAMORAK_POTION(1690)
		// ONE_ULTIMATE_ZAMORAK_POTION(1691)
	...
	private void usePrayerPotion(Player player, final Item item, final int newItem, final int left, final boolean sendUpdate, final double perc) {
		player.message("You drink some of your " + item.getDef(player.getWorld()).getName().toLowerCase());
		player.getCarriedItems().remove(item);
		player.getCarriedItems().getInventory().add(new Item(newItem));
		...
		for (int prayerId : prayerIds) {
			int newPrayer = player.getSkills().getLevel(prayerId) + (int) ((player.getSkills().getMaxStat(prayerId) * perc) + 7);
