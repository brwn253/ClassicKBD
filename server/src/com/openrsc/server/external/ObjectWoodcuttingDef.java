...
public final class ObjectWoodcuttingDef {
	...
	private int lowKhorium;
	private int highKhorium;
	private double[] rateKhorium;
	private int lowPlatinum;
	private int highPlatinum;
	private double[] ratePlatinum;
	private int lowTitanium;
	private int highTitanium;
	private double[] rateTitanium;
	...
	public double getRate(int level, int axe) {
		switch (axe) {
			...
			case 1512:
				return rateKhorium[level];
			case 1513:
				return ratePlatinum[level];
			case 1514:
				return rateTitanium[level];
		}
	}

	public void calculateWoodRates() {
		final int maxLevelToCalcFor = 150;
		...
		rateKhorium = new double[maxLevelToCalcFor];
		for (int level = 0; level < maxLevelToCalcFor; level++) {
			if (level >= requiredLevel) {
				rateKhorium[level] = Formulae.interp(lowKhorium, highKhorium, level);
			}
		}
		ratePlatinum = new double[maxLevelToCalcFor];
		for (int level = 0; level < maxLevelToCalcFor; level++) {
			if (level >= requiredLevel) {
				ratePlatinum[level] = Formulae.interp(lowPlatinum, highPlatinum, level);
			}
		}
		rateTitanium = new double[maxLevelToCalcFor];
		for (int level = 0; level < maxLevelToCalcFor; level++) {
			if (level >= requiredLevel) {
				rateTitanium[level] = Formulae.interp(lowTitanium, highTitanium, level);
			}
		}
	}
}
